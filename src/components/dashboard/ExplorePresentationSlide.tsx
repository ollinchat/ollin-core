...
const ALL_TAB: TabEntry = { id: "all", labelEn: "ALL", labelHe: "הכל" };
const DEALS: TabEntry = { id: "deals", labelEn: "Deals", labelHe: "מבצעים" };
const GOSSIP: TabEntry = { id: "gossip", labelEn: "Gossip", labelHe: "רכילות" };
const LOCAL_DEALS: TabEntry = { id: "local_deals", labelEn: "Local Deals", labelHe: "מבצעים לוקאליים" };
const JOBS: TabEntry = { id: "jobs", labelEn: "Jobs", labelHe: "משרות" };
const PROS: TabEntry = { id: "pros", labelEn: "Pros", labelHe: "מומחים" };

const TAB_LIBRARY: TabEntry[] = [DEALS, GOSSIP, LOCAL_DEALS, JOBS, PROS];

const INITIAL_TABS: TabEntry[] = [ALL_TAB, DEALS, GOSSIP, JOBS, PROS];
...
export function ExplorePresentationSlide() {
  ...
  const [tabs, setTabs] = useState<TabEntry[]>(INITIAL_TABS);
  ...
  const availableToAdd = useMemo(
    () => TAB_LIBRARY.filter((entry) => !tabs.some((t) => t.id === entry.id)),
    [tabs]
  );

  const removeTab = useCallback((currentId: TabId) => {
    if (currentId === "all") return;
    setTabs((prev) => prev.filter((t) => t.id !== currentId));
    setActiveTab((current) => (current === currentId ? "all" : current));
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="shrink-0 flex flex-col" style={{ backgroundColor: BG_WHITE }}>
        ...
        >
          <div
            ...
            style={{ backgroundColor: BG_SUBTLE }}
          >
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-sm font-medium ${!isHe ? "bg-gray-200 text-gray-800" : "text-gray-600 hover:bg-gray-100"}`}
            >
              EN
            </button>
            <button
        ...
        <section className="px-4 pt-4 pb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              className="flex items-center gap-1.5 shrink-0 px-3 py-2.5 rounded-xl border font-medium text-sm"
        ...
        {/* Tab Bar: X on each (except ALL), Emerald dashed "+", Edit/Done */}
        <div className="shrink-0 px-4 pb-3 overflow-x-auto scrollbar-hide">
          <div className="flex items-center gap-2 min-w-max">
            {tabs.map((tab) => (
              <div key={tab.id} className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="flex items-center gap-1.5 px-4 py-2.5 rounded-full text-sm font-semibold whitespace-nowrap border"
                  style={
                    activeTab === tab.id
                ...
                {isEditMode && tab.id !== "all" && (
                  <button
                    ...
                    className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-red-100 text-red-600"
            ...
            {isEditMode && (
              <div className="relative flex-shrink-0" ref={addMenuRef}>
                <button
                  ...
                  className="flex items-center justify-center w-10 h-10 rounded-full border-2 border-dashed"
                    ...
                    {availableToAdd.length === 0 ? (
                      ...
                      availableToAdd.map((entry) => (
                        <button
                          ...
                          className="w-full text-start px-3 py-2.5 text-sm font-medium hover:bg-gray-50 flex items-center gap-2"
            ...
            <button
              ...
              className={`flex-shrink-0 px-3 py-2 rounded-full text-xs font-medium ${isEditMode ? "bg-gray-800 text-white" : ""}`}
              style={!isEditMode ? { border: `1px dashed ${BORDER}`, color: TEXT_MUTED } : undefined}
            >
        ...
        {/* Scanning Hero: Radar + Progress (Ollin Emerald) */}
        <div
        ...
        >
          <div className="flex items-center gap-3 px-4 py-3.5">
            ...
            <button
              type="button"
              className="p-2 rounded-full hover:bg-white/80"
          ...
          <div className="px-4 pb-3.5">
            <div className="h-3 rounded-full overflow-hidden" style={{ backgroundColor: "rgba(0,0,0,0.1)" }}>
              <div
                ...
                style={{ width: `${scanProgress}%`, backgroundColor: OLLIN_EMERALD }}
              />
            </div>
            {scanComplete && (
      ...
      {/* Scrollable Area: ALL cards (Chargeback, Deals, News) — only this section scrolls */}
      <div
      ...
      >
        <div className="px-4 py-4 pb-6 space-y-4">
          ...
          {/* Deal card */}
          <article
            className="rounded-2xl overflow-hidden border shadow-md bg-white"
            style={{ borderColor: BORDER }}
          >
            <div className="relative aspect-[16/10] overflow-hidden">
              ...
              <div
                className={`absolute top-3 px-2.5 py-1.5 rounded-lg bg-black text-white text-xs font-black uppercase ${isHe ? "right-3" : "left-3"}`}
              >
          ...
          >
            <div className="relative aspect-[16/10] overflow-hidden">
              ...
              >
                <Newspaper className="w-14 h-14" style={{ color: TEXT_MUTED }} strokeWidth={1.5} />
              </div>
            </div>
            <div className="p-4">
              <h3 className="font-black text-base leading-snug" style={{ color: TEXT_PRIMARY }}>
                {isHe ? "חדשות מקומיות — אירועי השבוע" : "Local news — this week's events"}
              </h3>
              <p className="text-sm mt-1.5" style={{ color: TEXT_MUTED }}>
                {isHe ? "סיכום אירועים ופעילויות באזור." : "Summary of events and activities nearby."}
              </p>
            </div>
          </article>
        </div>
      ...
      {/* Bottom Fixed Area: Navigation */}
      <nav
        className="flex shrink-0 items-center justify-around py-3 border-t"
      ...
      >
        {[
          ...
          <button
            key={label}
            type="button"
            className="relative flex flex-col items-center gap-0.5 py-1 px-2 rounded-xl hover:bg-gray-50 min-w-[56px]"
          >
            <Icon
              className="w-6 h-6"
            ...
            />
            <span className="text-[10px] font-medium" style={{ color: active ? OLLIN_EMERALD : TEXT_MUTED }}>
              {label}
            </span>
            {active && (
              <span
