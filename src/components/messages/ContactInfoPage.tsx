...
/** Bottom sheet overlay + panel for Mute / Disappearing options */
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed left-0 right-0 bottom-0 z-50 rounded-t-3xl bg-white shadow-2xl border-t border-gray-200 font-sans"
          >
...
export function ContactInfoPage({ contactId }: ContactInfoPageProps) {
  ...
  const actionButtonClass =
    "flex flex-col items-center gap-3 flex-1 min-w-0 text-[#008080] transition-all duration-200 rounded-full group";
  const actionIconWrap =
    "relative w-14 h-14 rounded-full flex items-center justify-center shrink-0 backdrop-blur-md bg-white/70 border border-[#008080]/20 transition-all duration-200 group-hover:shadow-[0_0_20px_rgba(0,128,128,0.25)] group-hover:border-[#008080]/40";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50/80 font-sans">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {contact && (
          <div className="p-5 space-y-0">
            {/* 1. IDENTITY — Saved contact: name, mobile (+972), 7-digit ID. Primary headers for known contact. */}
            <GlassCard>
              <div className="p-6 flex flex-col items-center border-b border-gray-200/80">
                ...
                <p className="mt-5 text-lg font-semibold text-gray-900">{name}</p>
                <p className="mt-1 font-mono text-xl font-bold text-gray-900 tracking-tight">{mobileDisplay}</p>
            ...
            {/* 2. PREMIUM ACTION ROW — Ollin 6: Circle icons, backdrop-blur-md, turquoise glow on hover, single horizontal line, more spacing */}
            <GlassCard>
              <div className="flex flex-row items-center justify-around gap-4 px-6 py-6 border-b border-gray-200/80">
                <a href={contact.phone ? `tel:${contact.phone}` : "#"} className={actionButtonClass} aria-label="Audio">
                  <div className={actionIconWrap}>
                    <Phone className="w-6 h-6" strokeWidth={2} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Audio</span>
                </a>
                <button type="button" className={actionButtonClass} aria-label="Video + Screen Share">
                  <div className={actionIconWrap}>
                    <Video className="w-6 h-6" strokeWidth={2} />
                    <Monitor className="w-3.5 h-3.5 absolute -bottom-0.5 -right-0.5 text-[#008080]" strokeWidth={2.5} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Video</span>
                </button>
                <button type="button" className={actionButtonClass} aria-label="Pay">
                  <div className={actionIconWrap}>
                    <DollarSign className="w-6 h-6" strokeWidth={2} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Pay</span>
                </button>
                <button type="button" className={actionButtonClass} aria-label="Share">
                  <div className={actionIconWrap}>
                    <Share2 className="w-6 h-6" strokeWidth={2} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Share</span>
                </button>
                <Link href={`/dashboard/messages?contact=${contactId}`} className={actionButtonClass} aria-label="Message">
                  <div className={actionIconWrap}>
                    <MessageCircle className="w-6 h-6" strokeWidth={2} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Message</span>
                </Link>
                <button type="button" className={actionButtonClass} aria-label="Search">
                  <div className={actionIconWrap}>
                    <Search className="w-6 h-6" strokeWidth={2} />
                  </div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-gray-600">Search</span>
                </button>
              </div>
            </GlassCard>
            ...
            {/* 3. LIST — "Receive tasks from this user" (Checkmark) MUST BE FIRST. Then Mute (click → bottom sheet), Disappearing (click → bottom sheet), Encryption, Media, Profile, Board. */}
            <GlassCard>
              <div className="divide-y divide-gray-200/80">
                {/* FIRST: Task Permission — Checkmark, "Receive tasks from this user" [Toggle] */}
                ...
                {/* Mute Notifications — clickable row; opens Bottom Sheet with [8 Hours, 1 Week, Always] */}
                <button
                  type="button"
                ...
                {/* Disappearing Messages — clickable row; opens Bottom Sheet with [24 Hours, 7 Days, 90 Days, Off] */}
                <button
                  type="button"
                  onClick={() => setDisappearingSheetOpen(true)}
                  className="w-full flex items-center justify-between gap-3 px-4 py-4 hover:bg-white/50 transition-colors text-left border-b border-gray-200/80"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[#008080]/15 flex items-center justify-center text-[#008080] shrink-0">
            ...
            {/* 4. DANGER ZONE — Block [Name], Report [Name], red, separated */}
            <GlassCard>
              <div className="divide-y divide-gray-200/80">
                <button
                  type="button"
      ...
      {/* Mute Notifications — Bottom Sheet: 8 Hours, 1 Week, Always */}
      <BottomSheet open={muteSheetOpen} onClose={() => setMuteSheetOpen(false)} title={isHe ? "השתקת התראות" : "Mute Notifications"}>
        <div className="divide-y divide-gray-100">
          {(["8h", "1w", "always"] as const).map((opt) => (
            <button
              ...
              className="w-full px-4 py-4 text-left text-base font-medium text-gray-900 hover:bg-[#008080]/5 active:bg-[#008080]/10 transition-colors"
            >
      ...
      {/* Disappearing Messages — Bottom Sheet: 24 Hours, 7 Days, 90 Days, Off */}
      <BottomSheet open={disappearingSheetOpen} onClose={() => setDisappearingSheetOpen(false)} title={isHe ? "הודעות נעלמות" : "Disappearing Messages"}>
        <div className="divide-y divide-gray-100">
          {(["24h", "7d", "90d"] as const).map((opt) => (
            <button
              ...
              onClick={() => {
                ...
                setDisappearingSheetOpen(false);
              }}
              className="w-full px-4 py-4 text-left text-base font-medium text-gray-900 hover:bg-[#008080]/5 active:bg-[#008080]/10 transition-colors"
            >
          ...
          <button
            ...
            className="w-full px-4 py-4 text-left text-base font-medium text-gray-600 hover:bg-gray-50 active:bg-gray-100 transition-colors"
          >
            {isHe ? "כבוי" : "Off"}
          </button>
        </div>
