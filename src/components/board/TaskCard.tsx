...
export function TaskCard({ task, kind, selected, onToggleSelect, contacts = [] }: TaskCardProps) {
  ...
  const priorityBarColor =
    priority === "high" ? "bg-rose-400" : priority === "medium" ? "bg-amber-400" : "bg-[#008080]";
  const priorityMeta =
    priority === "high"
      ...
      : priority === "medium"
        ...
        : { label: locale === "he" ? "נמוך" : "Low", dotClass: "bg-[#008080]", textClass: "text-[#006666]" };
    ...
    >
      ...
      >
        {onToggleSelect != null && (
          <button
            ...
            className="flex-shrink-0 w-5 h-5 border border-[var(--clean-border)] flex items-center justify-center hover:border-slate-900 transition-colors text-[var(--clean-text-secondary)]"
            aria-label={selected ? "Deselect" : "Select for summary"}
          >
            {selected && <Check className="w-3 h-3 text-slate-900" strokeWidth={2} />}
          </button>
        )}
        <motion.span
        ...
        >
          <input
            ...
            className="w-4 h-4 border border-[var(--clean-border)] rounded focus:ring-0 accent-slate-900 checked:bg-slate-900 disabled:opacity-60 disabled:cursor-not-allowed"
          />
        </motion.span>
        ...
        <button
          ...
          className={`p-1.5 flex-shrink-0 transition-colors ${
            nudgeState.canNudge
              ? "text-[var(--clean-text-secondary)] hover:text-slate-900 hover:bg-slate-100"
        ...
        {expanded && (
          <>
            ...
            {task.done && task.doneNotifiedAt != null && (
              <span className="text-xs text-slate-600 flex items-center gap-0.5 flex-shrink-0" title="Sender notified">
        ...
        <div className="border-t border-[var(--clean-border)] px-4 py-3 pl-6 space-y-1.5 bg-white">
          {task.checklist.map((item) => (
            <div key={item.id} className="flex items-center gap-2">
              <input
                ...
                disabled={!canCheck}
                className="w-4 h-4 border-[var(--clean-border)] rounded accent-slate-900 checked:bg-slate-900"
              />
              {canEditAssign ? (
                <>
                  <input
                    ...
                    className="flex-1 min-w-0 text-[13px] border border-[var(--clean-border)] px-2 py-1 focus:border-slate-900 outline-none"
                  />
                  <UserSelector
                    users={internalUsers}
          ...
          {canEditAssign && (
            <div className="flex flex-wrap items-center gap-2">
              <button type="button" onClick={addChecklistItem} className="text-xs text-slate-600 hover:text-slate-900 hover:underline">
                + Add sub-task
              </button>
              <button
                type="button"
      ...
      {expanded && task.checklist.length === 0 && canEditAssign && (
        <div className="border-t border-[var(--clean-border)] px-4 py-3 pl-6 flex flex-wrap items-center gap-2">
          <button type="button" onClick={addChecklistItem} className="text-sm text-slate-600 hover:text-slate-900 hover:underline">
            + Add checklist
          </button>
          <button
            type="button"
        ...
        <div className="border-t border-[var(--clean-border)] px-4 py-3 pl-6 bg-white space-y-2">
          ...
          {comments.length > 0 ? (
            <ul className="space-y-1.5">
              {comments.map((c) => (
                <li key={c.id} className="text-xs text-[var(--clean-text)] pl-2 border-l-2 border-slate-300">
                  {c.text}
                </li>
              ))}
            </ul>
          ...
          {canComment && (
            <div className="flex gap-2 items-center">
              <input
                ...
                className="flex-1 min-w-0 text-[13px] border border-[var(--clean-border)] px-2 py-1.5 bg-white focus:border-slate-900 outline-none"
              />
              <MediaToolbox
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-900 text-white text-[13px] font-medium disabled:opacity-50 transition-colors rounded-md"
              >
                {locale === "he" ? "שלח" : "Send"}
              </button>
            </div>
